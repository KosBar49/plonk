/* Portable SHA-256. Public domain. Skipped on ESP32 where mbedtls is used. */
...
static void sha256_transform(plonk_sha256_ctx *ctx, const uint8_t *data) {
...
void plonk_sha256_finish(plonk_sha256_ctx *ctx, uint8_t out[32]) {
    ...
    for (int j = 0; j < 4; ++j) {
        for (int s = 0; s < 8; ++s) {
            out[j + s * 4] = (uint8_t)(ctx->state[s] >> (24 - j * 8));
        }
    }
}
